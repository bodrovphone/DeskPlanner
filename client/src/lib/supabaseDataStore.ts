...
export class SupabaseDataStore implements IDataStore {
  ...
  private mapToDatabase(booking: DeskBooking): any {
    ...
    const record: any = {
      ...
      is_ongoing: booking.isOngoing || false,
  ...
  async saveClient(client: Client): Promise<Client> {
    ...
    if (isNew) {
      const record: any = {
        ...
        payment_method_type: client.paymentMethodType || null,
        representative_name: client.representativeName || null,
        tax_id: client.taxId || null,
        vat_id: client.vatId || null,
  ...
  async endOngoingContract(args: {
    ...
    deleteQuery = numericClientId !== null
      ? deleteQuery.eq('client_id', numericClientId)
    ...
    updateQuery = numericClientId !== null
      ...
      : updateQuery.is('client_id', null);
    const { error: updateError } = await updateQuery;
    if (updateError) throw updateError;

    return { endedDate: args.newEndDate, rowsDeleted: rowsDeleted ?? 0 };
  }
