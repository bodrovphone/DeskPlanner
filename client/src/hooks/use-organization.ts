...
function mapOrg(row: Record<string, unknown>): Organization {
  return {
    ...
    billingLegalName: (row.billing_legal_name as string) ?? null,
    ...
    billingMol: (row.billing_mol as string) ?? null,
    billingCompiledBy: (row.billing_compiled_by as string) ?? null,
    billingBankDetails: (row.billing_bank_details as string) ?? null,
    defaultVatRate: (row.default_vat_rate as number) ?? 0,
    invoiceNumberNext: (row.invoice_number_next as number) ?? 1,
    invoiceNumberPadding: (row.invoice_number_padding as number) ?? 10,
